...
u(ki)=u(ki-1);
figure(1);hold on;
subplot(3,2,1);plot(t,hl,color);grid on;title('Altura','FontSize',TamanioFuente);hold on;
subplot(3,2,2);plot(t,fi_pl,color);grid on; 
title('$\dot{\phi_t}$','Interpreter','latex','FontSize',TamanioFuente);hold on;
subplot(3,2,3); plot(t,alfal,color);grid on;title('\alpha_t','FontSize',TamanioFuente);hold on;
subplot(3,2,4);plot(t,fil,color);grid on;title('\phi_t','FontSize',TamanioFuente);hold on;
